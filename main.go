...
func (t Training) meanSpeed() float64 {
	...
	d := t.distance()
	time := t.Duration / MinInHours

	return d / float64(time)
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	return CaloriesWeightMultiplier*float64(w.Weight) + ((w.meanSpeed()*KmHInMsec)/float64(w.Height))*CaloriesSpeedHeightMultiplier*float64(w.Weight)*float64(w.Duration)*MinInHours
}
...
func (s Swimming) meanSpeed() float64 {
	...
	return float64(s.LengthPool) * float64(s.Action) / MInKm / float64(s.Duration)
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	return (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * float64(s.Weight) * float64(s.Duration/MinInHours)
}
